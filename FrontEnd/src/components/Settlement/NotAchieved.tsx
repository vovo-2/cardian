...
export default function NotAchieved({
  userName,
  salary,
  onSetCheckCard,
  onSetCreditCard,
    ...
    <div>
      <div className="mt-10">
        ...
        <div className="flex justify-between">
          ...
          ) : accumulate / 10000 - Math.floor(accumulate / 100000000) * 10000 ==
            0 ? (
            <span>{Math.floor(accumulate / 100000000)}억원</span>
          ) : (
            <span>
              {Math.floor(accumulate / 100000000)}억
              {Math.floor(accumulate / 10000) -
                Math.floor(accumulate / 100000000) * 10000}
              만원
            </span>
          )}
          {goal < 100000000 ? (
            <span>{Math.floor(goal / 10000)}만원</span>
          ) : goal / 10000 - Math.floor(goal / 100000000) * 10000 == 0 ? (
            <span>{Math.floor(goal / 100000000)}억원</span>
          ) : (
            <span>
              {Math.floor(goal / 100000000)}억
              {Math.floor(goal / 10000) - Math.floor(goal / 100000000) * 10000}
              만원
            </span>
          )}
        </div>
      ...
      <div className="mt-5">
        <div className="text-3xl">
          <span className="font-semibold">{userName}</span>님은
        </div>
